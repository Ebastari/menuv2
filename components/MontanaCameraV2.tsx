  ...
  useEffect(() => {
    ...
    const startV2Logic = () => {
      ...
      async function capture(){
        ...
        if (isNaN(tahun)) { tahun = Math.max(2023, Math.min(2027, tahunSekarang)); showToast(`Tahun tanam diset ke: ${tahun}`, 0.8); } 
        else if (tahun < 2023) { tahun = 2023; showToast(`Tahun tanam diset ke min: ${tahun}`, 0.8); } 
        else if (tahun > 2027) { tahun = 2027; showToast(`Tahun tanam diset ke max: ${tahun}`, 0.8); }
        tahunInput.value = String(tahun);

        if (lastGPS.akurasi > 10) showToast(`Akurasi kurang bagus (${lastGPS.akurasi}m).`, 0.5);
      ...
      btnGallery.addEventListener('click', ()=> sheet.classList.toggle('visible'));
      ...
      handle.addEventListener('click', ()=> sheet.classList.toggle('visible'));
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black animate-fadeIn overflow-hidden">
      <style>{`
        ...
        .v2-app-scope .gps-good .dot { background-color: var(--accent); }
        .v2-app-scope .gps-medium .dot { background-color: var(--warn); }
        .v2-app-scope .gps-bad .dot { background-color: var(--danger); }
