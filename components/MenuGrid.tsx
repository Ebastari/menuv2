...
interface MenuGridProps {
  ...
  onOpenBibitNotification: () => void;
  onRequestLogin: () => void;
}

const GUEST_ALLOWED_IDS = ['carbon', 'height', 'news-2025', 'weather', 'peta'];

export const MenuGrid: React.FC<MenuGridProps> = ({ role, onOpenMontana, onOpenDashboardAI, onOpenDeveloperInfo, onOpenBibitNotification, onRequestLogin }) => {
  ...
  const handleMenuClick = (e: React.MouseEvent, item: MenuItem) => {
    ...
    if (item.id === 'developer-info') {
      onOpenDeveloperInfo();
      return;
    }

    if (item.id === 'bibit-notification') {
      onOpenBibitNotification();
      return;
    }
